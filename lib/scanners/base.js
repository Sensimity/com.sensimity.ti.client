...
var BaseScanner = function(beaconMapper, beaconRegionMapper, beaconRegionMonitoringMapper) {
    ...
    this.init = function (networkIdentifier) {
        if (!_.isUndefined(networkIdentifier)) {
            ...
            if (OS_IOS) {
                ...
                initIOSBackgroundService();
            }

            beaconHandler.init();
            beaconLog.init();
            knownBeaconService.refreshBeacons([networkId]);
        } else {
    ...
    this.startScanning = function() {
        self.bindService();
        var knownBeacons = knownBeaconService.getKnownBeacons(networkId);
    ...
    function startScanningOfBeacon(knownBeacon) {
        ...
        if (checkBeaconRegionNotScanning(knownBeacon)) {
     ...
     */
    function checkBeaconRegionNotScanning(knownBeacon) {
        // Check beaconregion already scanning
        var foundBeaconRegion = _.find(beaconRegions, function(region) {
            return _.isEqual(region.uuid.toUpperCase(), knownBeacon.get('UUID').toUpperCase());
        });
        return _.isUndefined(foundBeaconRegion);
    }

    // Initialize the backgroundservice for iOS
    function initIOSBackgroundService() {
        if (OS_IOS) {
            Ti.App.iOS.registerBackgroundService({
                url: './../service/handleBackgroundScan.js'
            });
        }
    }
};
