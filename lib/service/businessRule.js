    ...
    sensimityClient.getBusinessRules(knownBeacon.get('network_id'), knownBeacon.get('beacon_id'), function (data) {
        if (!_.isEmpty(data._embedded.business_rule)) {
            _.each(data._embedded.business_rule, function (businessruleRaw) {
                fetchSingleBusinessRule(knownBeacon, businessruleRaw.business_rule_id);
            });
        }
    });
}
...
/******
 * Function to fetch a single business Rule by a beacon and businessrule-id
 * @param {Object} knownBeacon find the business rule of this beacon
 * @param {Object} businessruleId find the business rules of this business-rule-id
 */
function fetchSingleBusinessRule(knownBeacon, businessruleId) {
    sensimityClient.getSingleBusinessRule(knownBeacon.get('network_id'), businessruleId, saveFetchedBusinessRules);
}
