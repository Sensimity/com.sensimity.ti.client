 ...
 * @param onFailure {callback when starting sensimity failed}
 */
function start(options, onFailure) {
    ...
    isBLEEnabled(function (value) {
        ...
        if (_.isUndefined(Alloy.Globals.sensimityScanner) === false) {
            var message = 'Scanner already defined, please destruct first before start scanning';
            Ti.API.warn(message);
            if (_.isFunction(onFailure)) {
                onFailure({
                    message: message
 ...
 * @param onFailure {callback when starting sensimity failed}
 */
function runService(options, onFailure) {
    // Only run Sensimity-service when bluetooth is enabled
    isBLEEnabled(function (value) {
        if (!value) {
            var message = 'Sensimity scan not started because BLE not enabled';
            Ti.API.warn(message);
