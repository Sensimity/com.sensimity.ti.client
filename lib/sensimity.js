...
function start(params) {
    ...
    if (!options.backgroundMode && OS_ANDROID) {
 ...
 */
function stop() {
    Alloy.Globals.sensimityEvent.off('sensimity:beaconsRefreshed', handleBeaconRefresh);
...
function startBackgroundIntent() {
    if (OS_ANDROID) {
        Alloy.Globals.scanIntent = Ti.Android.createServiceIntent({
            url: 'services/handleBackgroundScan.js'
        });
...
function initScannerAndStartScanning(options) {
    if (!_.isUndefined(options.networkId) && !_.isNull(options.networkId)) {
        ...
        Alloy.Globals.sensimityEvent.on('sensimity:beaconsRefreshed', handleBeaconRefresh);
    } else {
